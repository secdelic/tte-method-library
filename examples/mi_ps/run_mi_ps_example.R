...
example_root <- file.path(project_root, "examples", "mi_ps")
...
if (!isTRUE(result$gate$diagnostic_gate_pass)) {
  stop("Synthetic MI+PS diagnostic gate failed: ", result$gate$stopping_reasons)
}
message("Synthetic MI+PS production profile PASS")
